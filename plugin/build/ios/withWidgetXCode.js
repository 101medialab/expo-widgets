...
const addFilesToWidgetProject = (project, { widgetFolderPath, iosProjectPath, targetUuid, targetName, projectName, expoConfig, options, }) => {
    const iterateFiles = (relativePath, isBaseDirectory, parentGroup, widgetTargetUuid) => {
        ...
        const shouldAddResourcesBuildPhase = () => {
            ...
            return fs_1.default.existsSync(googleServicePlistPath) || filesByType.xcassets?.length > 0 || filesByType.strings?.length > 0 || filesByType.xcstrings?.length > 0;
        };
        ...
        if (isBaseDirectory) {
            ...
            project.addBuildPhase(filesByType.swift, "PBXSourcesBuildPhase", groupName, widgetTargetUuid, "app_extension", // folder type
            "");
        ...
        if (filesByType.xcstrings) {
            for (const xcstringsFile of filesByType.xcstrings) {
                logger_1.Logging.logger.debug(`Adding xcstrings file:: ${xcstringsFile} to target ${targetUuid}`);
